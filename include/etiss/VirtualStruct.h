    ...
    */
    class Field
    {
      ...
      public: // read write
        ...
        void write(uint64_t,
                   size_t offset = 0); ///< function to write bits/a value to the Field. may only be called if the W
                                       ///< flag is set. additional bits are silently discarded if the targt field has
                                       ///< less than 64 bits. e.g. int64_t write_target = (int32_t) write_uint64_t;
    ...
    class FieldT : public VirtualStruct::Field
    {
      ...
      protected:
        ...
        virtual uint64_t _read(size_t offset = 0) const
        {
            ...
            assert((offset == 0 || (offset < (bitwidth_ / sizeof(uint64_t)))) &&
                   "Virtualstruct field offset out of range");
