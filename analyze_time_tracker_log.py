...
base_name = log_file.stem  # Get filename without extension
...
plt.tight_layout()
# plt.show()
plt.savefig(output_dir / f"{base_name}_mips_plot.png", dpi=300)
plt.close()
...
# Save binned time data
# bin_time.to_csv(output_dir / f"{base_name}_binned_time.csv", index=False)
...
# plt.grid(True)
plt.tight_layout()
plt.savefig(output_dir / f"{base_name}_time_per_bin.png", dpi=300)
plt.close()

# Save summary statistics
with open(output_dir / f"{base_name}_summary.txt", "w") as f:
    f.write(f"Mean MIPS: {mean_mips2:.2f}\n")
    f.write(f"Total simulation time: {total_delta_time:.2f}s\n")
    f.write(f"Total instructions: {bin_time['start_instr'].max()}\n")
