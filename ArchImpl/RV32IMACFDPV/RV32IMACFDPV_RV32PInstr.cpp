...
partInit.code() += "*((RV32IMACFDPV*)cpu)->X[" + std::to_string(rd * 2U + 1U) + "] = (((product) >> (32U)) & 4294967295);\n";
partInit.code() += "*((RV32IMACFDPV*)cpu)->X[" + std::to_string(rd * 2U) + "] = (((product) >> (0U)) & 4294967295);\n";
// -----------------------------------------------------------------------------

		partInit.getRegisterDependencies().add(reg_name[rs1], 32);
		partInit.getRegisterDependencies().add(reg_name[rs2], 32);
		partInit.getAffectedRegisters().add(reg_name[rd * 2U], 32);
		partInit.getAffectedRegisters().add(reg_name[rd * 2U + 1U], 32);
		partInit.getAffectedRegisters().add("instructionPointer", 32);
