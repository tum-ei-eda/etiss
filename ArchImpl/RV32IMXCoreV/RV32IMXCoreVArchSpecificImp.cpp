 ...
 *
 * This file contains the architecture specific implementation for the RV32IMXCoreV
 * core architecture.
 *
...
#include "RV32IMXCoreVArchSpecificImp.h"
...
*/
etiss::int32 RV32IMXCoreVArch::handleException(etiss::int32 cause, ETISS_CPU *cpu)
{
    RV32IMXCoreV_translate_exc_code(cpu, nullptr, nullptr, cause);
...
*/
std::string RV32IMXCoreVArch::installDir() const
{
...
*/
std::string RV32IMXCoreVArch::jitFiles() const
{
...
*/
void RV32IMXCoreVArch::initInstrSet(etiss::instr::ModedInstructionSet &mis) const
{
    {
        ...
        etiss::cfg().set<std::string>("jit.external_headers", cfgPar + "");

        cfgPar = etiss::cfg().get<std::string>("jit.external_libs", ";");
        etiss::cfg().set<std::string>("jit.external_libs", cfgPar + "");
    ...
    if (false) {
        ...
        etiss::instr::ModedInstructionSet iset("RV32IMXCoreVISA");
        bool ok = true;
        RV32IMXCoreVISA.addTo(iset, ok);
    ...
    bool ok = true;
    RV32IMXCoreVISA.addTo(mis, ok);
    if (!ok)
        etiss::log(etiss::FATALERROR, "Failed to add instructions for RV32IMXCoreVISA");
...
{ // procedure
cp.code() += "{ // procedure\n";
cp.code() += "RV32IMXCoreV_translate_exc_code(cpu, system, plugin_pointers, " + std::to_string(error_code) + "ULL);\n";
...
// -----------------------------------------------------------------------------
    ...
    vis->length_updater_ = [](VariableInstructionSet &, InstructionContext &ic, BitArray &ba) {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateRV32IMXCoreVInstrLength =
        ...
        case 2:
            ...
            {
                updateRV32IMXCoreVInstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void RV32IMXCoreVArch::compensateEndianess(ETISS_CPU *cpu, etiss::instr::BitArray &ba) const
{
    /**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> RV32IMXCoreVArch::getVirtualStruct(ETISS_CPU *cpu)
{
    auto ret = etiss::VirtualStruct::allocate(cpu, [](etiss::VirtualStruct::Field *f) { delete f; });

    for (uint32_t i = 0; i < 32; i += 1)
    {
        ret->addField(new RegField_RV32IMXCoreV(*ret, i));
    }
    for (uint32_t i = 768; i < 775; i += 1)
    {
        ret->addField(new CSRField_RV32IMXCoreV(*ret, i));
    }
    for (uint32_t i = 832; i < 837; i += 1)
    {
        ret->addField(new CSRField_RV32IMXCoreV(*ret, i));
    }
    ret->addField(new CSRField_RV32IMXCoreV(*ret, 2816));
    ret->addField(new CSRField_RV32IMXCoreV(*ret, 2818));
    ret->addField(new CSRField_RV32IMXCoreV(*ret, 2944));
    ret->addField(new CSRField_RV32IMXCoreV(*ret, 2946));
    ...
    ret->addField(new pcField_RV32IMXCoreV(*ret));
...
/**
    ...
    @details Interrupt vector table is used to inform the core whenever an edge/level triggered interrupt
                ...
                lines.
*/
etiss::InterruptVector *RV32IMXCoreVArch::createInterruptVector(ETISS_CPU *cpu)
{
    if (cpu == 0)
        return 0;
    ...
    vec.push_back(&((RV32IMXCoreV *)cpu)->MIP);
    mask.push_back(&((RV32IMXCoreV *)cpu)->MIE);

    return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void RV32IMXCoreVArch::deleteInterruptVector(etiss::InterruptVector *vec, ETISS_CPU *cpu)
{
    delete vec;
}

etiss::InterruptEnable *RV32IMXCoreVArch::createInterruptEnable(ETISS_CPU *cpu)
{
    return new etiss::MappedInterruptEnable<etiss::uint32>(&((RV32IMXCoreV *)cpu)->MSTATUS, 15);
}

void RV32IMXCoreVArch::deleteInterruptEnable(etiss::InterruptEnable *en, ETISS_CPU *cpu)
{
    delete en;
}
