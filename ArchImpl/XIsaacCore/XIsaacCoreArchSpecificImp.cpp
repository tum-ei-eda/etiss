 ...
 *
 * This file contains the architecture specific implementation for the XIsaacCore
 * core architecture.
 *
...
#include "XIsaacCoreArchSpecificImp.h"
...
*/
etiss::int32 XIsaacCoreArch::handleException(etiss::int32 cause, ETISS_CPU *cpu)
{
    XIsaacCore_translate_exc_code(cpu, nullptr, nullptr, cause);
...
*/
void XIsaacCoreArch::initInstrSet(etiss::instr::ModedInstructionSet &mis) const
{
    ...
    if (false)
    {
        ...
        etiss::instr::ModedInstructionSet iset("XIsaacCoreISA");
        bool ok = true;
        XIsaacCoreISA.addTo(iset, ok);
    ...
    bool ok = true;
    XIsaacCoreISA.addTo(mis, ok);
    if (!ok)
        etiss::log(etiss::FATALERROR, "Failed to add instructions for XIsaacCoreISA");
    ...
    vis->get(32)->getInvalid().addCallback(
        [](BitArray &ba, etiss::CodeSet &cs, InstructionContext &ic)
        {
...

            {
                ...
                { // procedure
                    cp.code() += "{ // procedure\n";
                    cp.code() += "XIsaacCore_translate_exc_code(cpu, system, plugin_pointers, " +
    ...
    {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateXIsaacCoreInstrLength =
        ...
        case 2:
            ...
            {
                updateXIsaacCoreInstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void XIsaacCoreArch::compensateEndianess(ETISS_CPU *cpu, etiss::instr::BitArray &ba) const
{
    /**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> XIsaacCoreArch::getVirtualStruct(ETISS_CPU *cpu)
{
    auto ret = etiss::VirtualStruct::allocate(cpu, [](etiss::VirtualStruct::Field *f) { delete f; });

    for (uint32_t i = 0; i < 32; ++i)
    {
        ret->addField(new RegField_XIsaacCore(*ret, i));
    }

    ret->addField(new pcField_XIsaacCore(*ret));
    return ret;
}
...
*/
etiss::InterruptVector *XIsaacCoreArch::createInterruptVector(ETISS_CPU *cpu)
{
    if (cpu == 0)
        return 0;
    ...
    vec.push_back(&((XIsaacCore *)cpu)->MIP);
    mask.push_back(&((XIsaacCore *)cpu)->MIE);

    return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void XIsaacCoreArch::deleteInterruptVector(etiss::InterruptVector *vec, ETISS_CPU *cpu)
{
    delete vec;
}

etiss::InterruptEnable *XIsaacCoreArch::createInterruptEnable(ETISS_CPU *cpu)
{
    return new etiss::MappedInterruptEnable<etiss::uint32>(&((XIsaacCore *)cpu)->MSTATUS, 15);
}

void XIsaacCoreArch::deleteInterruptEnable(etiss::InterruptEnable *en, ETISS_CPU *cpu)
{
    delete en;
}
