...
void PrintInstruction::finalizeInstrSet(etiss::instr::ModedInstructionSet &mis) const
{
    ...
    mis.foreach (
        [print_to_file](etiss::instr::VariableInstructionSet &vis)
        {
            vis.foreach (
                [print_to_file](etiss::instr::InstructionSet &set)
                {
                    set.foreach (
                        [print_to_file](etiss::instr::Instruction &instr)
                        {
                            instr.addCallback(
                                [&instr,print_to_file](etiss::instr::BitArray &ba, etiss::CodeSet &cs,
