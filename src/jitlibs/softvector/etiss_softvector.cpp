...
extern "C"
{
                                           ...
                                           uint16_t pVL, uint64_t pMSTART)
    {
        ...
        if (pVm)
        {
            VLSU::load_unitstride<false>(f_readMem, VectorRegField, eew_bytes, pVL, pVLEN / 8, pVd, pMSTART, pVSTART);
        }
        else
        {
            VLSU::load_unitstride<true>(f_readMem, VectorRegField, eew_bytes, pVL, pVLEN / 8, pVd, pMSTART, pVSTART);
        }
        // VLSU::load_eew(f_readMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8, pVd, pMSTART, pVSTART,
        //                pVm, eew_bytes);
        return (0);
    }
                             ...
                             uint64_t pMSTART)
    {
        ...
        if (pVm)
        {
            VLSU::load_eew_v2<false>(f_readMem, VectorRegField, eew_bytes, evl, pVLEN / 8, pVd, pMSTART, pVSTART,
                                     eew_bytes);
        }
        else
        {
            VLSU::load_eew_v2<true>(f_readMem, VectorRegField, eew_bytes, evl, pVLEN / 8, pVd, pMSTART, pVSTART,
                                    eew_bytes);
        }
        // VLSU::load_eew(f_readMem, VectorRegField, emul_num, emul_denom, eew_bytes, evl, pVLEN >> 3, pVd, pMSTART,
        //                pVSTART, pVm, eew_bytes);
        return 0;
    }
                                       ...
                                       uint64_t pMSTART, int16_t pSTRIDE)
    {
        ...
        if (pVm)
        {
            VLSU::load_eew_v2<false>(f_readMem, VectorRegField, eew_bytes, pVL, pVLEN / 8, pVd, pMSTART, pVSTART,
                                     pSTRIDE);
        }
        else
        {
            VLSU::load_eew_v2<true>(f_readMem, VectorRegField, eew_bytes, pVL, pVLEN / 8, pVd, pMSTART, pVSTART,
                                    pSTRIDE);
        }
        // VLSU::load_eew(f_readMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8, pVd, pMSTART, pVSTART,
        //                pVm, pSTRIDE);
                                      ...
                                      uint64_t pMSTART, uint8_t pVs2)
    {
        ...
        auto v_instr_info = VInstrInfo{};

        v_instr_info.lmul_num = _vt._z_lmul;
        v_instr_info.lmul_denom = _vt._n_lmul;
        v_instr_info.sew = _vt._sew;
        v_instr_info.vector_length = pVL;
        v_instr_info.vector_register_length = pVLEN;
        v_instr_info.start_element = pVSTART;
        v_instr_info.masked = !pVm;

        // if (pVm)
        // {
        //     VLSU::load_indices_v2<false>(f_readMem, VectorRegField, v_instr_info, pVd, pVs2, pMSTART, pEEW);
        // }
        // else
        // {
        //     VLSU::load_indices_v2<true>(f_readMem, VectorRegField, v_instr_info, pVd, pVs2, pMSTART, pEEW);
        // }
                                           ...
                                           uint16_t pVLEN, uint16_t pVL, uint64_t pMSTART)
    {
        ...
        if (pVm)
        {
            ...
            {
                VLSU::load_eew_v2<false>(f_readMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                         pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), _moffset, _vstart,
                                         pNF * eew_bytes);
                _moffset += eew_bytes;
            }
        }
        else
        {
            for (size_t i = 0; i < pNF; ++i)
            {
                VLSU::load_eew_v2<true>(f_readMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                        pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), _moffset, _vstart,
                                        pNF * eew_bytes);
                _moffset += eew_bytes;
            }
        ...
        // for (size_t i = 0; i < pNF; ++i)
        //     {
        //         VLSU::load_eew(f_readMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8,
        //                        pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), _moffset, _vstart, pVm,
        //                        pNF * eew_bytes);
        //         _moffset += eew_bytes;
        //         // _vstart = 0;
        //     }
                                       ...
                                       uint16_t pVL, uint64_t pMSTART, int16_t pSTRIDE)
    {
        ...
        else
        {
            for (size_t i = 0; i < pNF; ++i)
            {
                _moffset = pMSTART + i * (eew_bytes);
                VLSU::load_eew_v2<true>(f_readMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                        pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), _moffset, _vstart, pSTRIDE);
                // _vstart = 0;
            }
        }
        // for (size_t i = 0; i < pNF; ++i)
        ...
        //     _moffset = pMSTART + i * (eew_bytes);
        //     VLSU::load_eew(f_readMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8,
        //                    pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), _moffset, _vstart, pVm, pSTRIDE);
        //     // _vstart = 0;
        // }
        return (0);
    }
                                      ...
                                      uint16_t pVL, uint64_t pMSTART, uint8_t pVs2)
    {
        ...
        //     for (size_t i = 0; i < pNF; ++i)
        //     {
        //         VLSU::load_indices_v2<false>(f_readMem, VectorRegField, v_instr_info,
        //                                      pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), pVs2, _moffset,
        //                                      pEEW);
        //         _moffset += _vt._sew >> 3;
        //     }
        // }
        ...
        //     for (size_t i = 0; i < pNF; ++i)
        //     {
        //         VLSU::load_indices_v2<true>(f_readMem, VectorRegField, v_instr_info,
        //                                     pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), pVs2, _moffset,
        //                                     pEEW);
        //         _moffset += _vt._sew >> 3;
        //     }
        ...
        for (size_t i = 0; i < pNF; ++i)
        {
            VLSU::load_indices(f_readMem, VectorRegField, v_instr_info,
                               pVd + std::max(i, i * (_vt._z_lmul / _vt._n_lmul)), pVs2, _moffset, pEEW);
            _moffset += _vt._sew >> 3;
        }
        return (0);
    }
                                  ...
                                  uint64_t pMSTART)
    {
        ...
        // VLSU::load_eew(f_readMem, VectorRegField, _z_emul, _n_emul, eew_bytes, evl, pVLEN / 8, pVd, pMSTART, pVSTART,
        //                pVm, eew_bytes);
        return (0);
    }
                                            ...
                                            uint16_t pVL, uint64_t pMSTART)
    {
        ...
        if (pVm)
        {
            VLSU::store_eew_v2<false>(f_writeMem, VectorRegField, eew_bytes, pVL, pVLEN / 8, pVs3, pMSTART, pVSTART,
                                      eew_bytes);
        }
        else
        {
            VLSU::store_eew_v2<true>(f_writeMem, VectorRegField, eew_bytes, pVL, pVLEN / 8, pVs3, pMSTART, pVSTART,
                                     eew_bytes);
        }
        // VLSU::store_eew(f_writeMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8, pVs3, pMSTART,
        // pVSTART,
        //                 pVm, eew_bytes);
                              ...
                              uint64_t pMSTART)
    {
        ...
        if (pVm)
        {
            VLSU::store_eew_v2<false>(f_writeMem, VectorRegField, eew_bytes, evl, pVLEN / 8, pVs3, pMSTART, pVSTART,
                                      eew_bytes);
        }
        else
        {
            VLSU::store_eew_v2<true>(f_writeMem, VectorRegField, eew_bytes, evl, pVLEN / 8, pVs3, pMSTART, pVSTART,
                                     eew_bytes);
        }
        // VLSU::store_eew(f_writeMem, VectorRegField, emul_num, emul_denom, eew_bytes, evl, pVLEN >> 3, pVs3, pMSTART,
        //                 pVSTART, pVm, eew_bytes);
                                       ...
                                       uint64_t pMSTART, uint8_t pVs2)
    {
        ...
        // if (pVm)
        // {
        //     VLSU::store_indices_v2<false>(f_writeMem, VectorRegField, v_instr_info, pVs3, pVs2, pMSTART, pEEW, 1);
        // }
        // else
        // {
        //     VLSU::store_indices_v2<true>(f_writeMem, VectorRegField, v_instr_info, pVs3, pVs2, pMSTART, pEEW, 1);
        // }
                                            ...
                                            uint16_t pVLEN, uint16_t pVL, uint64_t pMSTART)
    {
        ...
        if ((_n_emul > _z_emul * pNF * 8) || (_z_emul * pNF > _n_emul * 8))
            return 1;
        if ((pVs3 + pNF * _z_emul / _n_emul) > 32)
            return 1;
        if (pVSTART >= pVL)
            return (0);
        ...
        if (pVm)
        {
            for (size_t i = 0; i < pNF; ++i)
            {
                VLSU::store_eew_v2<false>(f_writeMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                          pVs3 + std::max(i, (i * _z_emul / _n_emul)), _moffset, _vstart,
                                          pNF * eew_bytes);
                _moffset += eew_bytes;
        ...
        else
        {
            for (size_t i = 0; i < pNF; ++i)
            {
                VLSU::store_eew_v2<true>(f_writeMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                         pVs3 + std::max(i, (i * _z_emul / _n_emul)), _moffset, _vstart,
                                         pNF * eew_bytes);
                _moffset += eew_bytes;
                // TODO: Why reset _vstart?
        ...
        // for (size_t i = 0; i < pNF; ++i)
        // {
        //     VLSU::store_eew(f_writeMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8,
        //                     pVs3 + std::max(i, (i * _z_emul / _n_emul)), _moffset, _vstart, pVm, pNF * eew_bytes);
        //     _moffset += eew_bytes;
        //     // TODO: Why reset _vstart?
        //     // _vstart = 0;
        // }
                                        ...
                                        uint16_t pVL, uint64_t pMSTART, int16_t pStride)
    {
        ...
        if (pStride < 0)
        {
            if (pVm)
            {
                ...
                {
                    VLSU::store_eew_v2<false>(f_writeMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                              pVs3 + std::max(i, (i * _z_emul / _n_emul)), _moffset, _vstart, pStride);
                    _moffset += eew_bytes;
                }
            }
            else
        ...
        else
        {
            if (pVm)
            {
                for (size_t i = pNF; i > 0; --i)
                {
                    ...
                    VLSU::store_eew_v2<false>(f_writeMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                              pVs3 + std::max((i - 1), ((i - 1) * _z_emul / _n_emul)), _moffset,
                                              _vstart, pStride);
                }
            }
            else
            {
                for (size_t i = pNF; i > 0; --i)
                {
                    _moffset = pMSTART + ((i - 1) * eew_bytes);
                    VLSU::store_eew_v2<true>(f_writeMem, VectorRegField, eew_bytes, pVL, pVLEN / 8,
                                             pVs3 + std::max((i - 1), ((i - 1) * _z_emul / _n_emul)), _moffset, _vstart,
                                             pStride);
                }
            }
        }
        ...
        //     {
        //         VLSU::store_eew(f_writeMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8,
        //                         pVs3 + std::max(i, (i * _z_emul / _n_emul)), _moffset, _vstart, pVm, pStride);
        //         _moffset += eew_bytes;
        //     }
        // }
        // else
        ...
        //         _moffset = pMSTART + ((i - 1) * eew_bytes);
        //         VLSU::store_eew(f_writeMem, VectorRegField, _z_emul, _n_emul, eew_bytes, pVL, pVLEN / 8,
        //                         pVs3 + std::max((i - 1), ((i - 1) * _z_emul / _n_emul)), _moffset, _vstart, pVm,
        //                         pStride);
        //     }
        // }
        return (0);
                                       ...
                                       uint16_t pVL, uint64_t pMSTART, uint8_t pVs2)
    {
        ...
        // if (pVm)
        // {
        //     VLSU::store_indices_v2<false>(f_writeMem, VectorRegField, v_instr_info, pVs3, pVs2, pMSTART, pEEW, pNF);
        // }
        // else
        // {
        //     VLSU::store_indices_v2<true>(f_writeMem, VectorRegField, v_instr_info, pVs3, pVs2, pMSTART, pEEW, pNF);
        // }
                                   ...
                                   uint8_t pNF, uint8_t pVs3, uint16_t pVSTART, uint16_t pVLEN, uint64_t pMSTART)
    {
        ...
        // if (pVm)
        // {
        //     VLSU::store_eew_v2<false>(f_writeMem, VectorRegField, pNF, evl, pVLEN / 8, pVs3, pMSTART, pVSTART, 1);
        // }
        // else
        // {
        //     VLSU::store_eew_v2<true>(f_writeMem, VectorRegField, pNF, evl, pVLEN / 8, pVs3, pMSTART, pVSTART, 1);
        // }
