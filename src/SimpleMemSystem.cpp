...
void access_error(ETISS_CPU *cpu, etiss::uint64 addr, etiss::uint32 len, std::string error, etiss::Verbosity verbosity, int message_max_cnt)
{
...
etiss::int32 SimpleMemSystem::dbus_access(ETISS_CPU *cpu, etiss::uint64 addr, etiss::uint8 *buf, etiss::uint32 len)
{
    ...
    access_error(cpu, addr, len, std::string("dbus ") + (write ? "write" : "read") + " error", verbosity, message_max_cnt_);
...
void
#endif
SimpleMemSystem::syncTime(ETISS_CPU *cpu)
{
