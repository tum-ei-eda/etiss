...
void etiss_initialize(const std::vector<std::string> &args, bool forced = false)
{
...

    {
        ...
        try
        {
            ...
            ("vp.print_exit_status", po::value<bool>(), "Print CPU exit status after simulation finish.")
            ("vp.check_exit_status", po::value<bool>(), "Raise error if CPU exit status is non zero.")
            ("vp.forward_exit_status", po::value<bool>(), "Forward CPU exit code to shell.")
            ("vp.progname", po::value<std::string>(), "Command line program.")
            ("vp.cmdline", po::value<std::string>(), "Command line arguments.")
