...
void etiss_initialize(const std::vector<std::string> &args, bool forced = false)
{
...

    {
        ...
        try
        {
            ...
            ("etiss.exit_on_loop", po::value<bool>(), "Exit on loop to self.")
            ...
            ("vp.coredsl_coverage_path", po::value<std::string>(), "The coverage output file.")
            ("vp.entry_point", po::value<uint64_t>(), "The entry point address.")
            ...
            ("simple_mem_system.error_on_seg_mismatch", po::value<bool>(), "Exit on segment missmatch.")
            ("simple_mem_system.error_on_invalid_access", po::value<bool>(), "Exit on invalid access.")
            ("simple_mem_system.message_max_cnt", po::value<int>(), "Limit repeated messages.")
            ...
            ("plugin.printinstruction.print_to_file", po::value<bool>(), "Write instruction trace to instr_trace.csv file. Use etiss.output_path_prefix to override directory.")
            ;
            // clang-format on
